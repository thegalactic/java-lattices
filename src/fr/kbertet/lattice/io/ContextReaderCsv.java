...
public final class ContextReaderCsv implements ContextReader {
    ...
    public void read(Context context, BufferedReader file) throws IOException {
        ...
        if (size < 2) {
            throw new IOException("Attributes size cannot be 0");
        }

        if (!attributes.get(0).equals("")) {
            throw new IOException("The first line contains a name for the identifier");
        }

        for (int i = 1; i < size; i++) {
            if (!context.addToAttributes(attributes.get(i))) {
                throw new IOException("Duplicate attribute");
            }
        }
